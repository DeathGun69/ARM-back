...
function page(keys_all,token,table_name){
  ...
  $.ajax({
    ...
    success:function(data){/*
      ...
      console.log(keys)
      $( ".page-wrap .container-fluid" ).append('<div class="row"> <table class="table table-striped table-hover"><div class="title"> <h1>'+keys_all[table_name]+'</h1></div> <thead class="tabt">   <tr ><th>назание</th><th><div class="cl"></div></th>  </tr>  </thead> <tbody class="tabb"></tbody></table> </div>   ')
         

          for(i=0;i<data.result.length;i++){
            $( ".tabb" ).append( '<tr id="tr'+data.result[i].id+'" class=>')
            $( '#tr'+data.result[i].id ).append( '<th>'+data.result[i].name+'</th>' )
            $( "#tr"+data.result[i].id ).append( '<th><button class="buttonr recours" value='+data.result[i].id+'>изменить</button><button class="del" value='+data.result[i].id+'>удалить</button></th>' ) 
